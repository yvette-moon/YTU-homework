...
bool backtrack(vector<int>& sticks, vector<bool>& used, int target, int current, int k, int start) {
    if (k == 0) {
...
int main() {
    ...
    while (cin >> n) {
        ...
        for (int i = 1; i <= sum; i++) {
            if (sum % i == 0) { // 检查是否可以分成每组长度为 i 的棍子
                vector<bool> used(n, false);
                if (backtrack(sticks, used, sum / i, 0, i, 0)) {
                    cout << i << endl;
                    break;
                }
            }
